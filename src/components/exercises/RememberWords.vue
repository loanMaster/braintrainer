<template>
  <div ref="buttons" class="row q-gutter-sm justify-center">
    <div v-for="(label, idx) in buttonLabels" v-bind:key="idx" class="row">
      <q-btn color="primary" :class="textTransparent ? 'text-transparent' : ''"
             @click="selectWord(idx, $event)" :disabled="isButtonDisabled(idx)" class="transition-duration-md"
      >{{label}}</q-btn>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const sequenceLength = computed(() => {
    return difficulty.value === 'easy' ? 6 : difficulty.value === 'normal' ? 9 : 12
  })
  ...
  async function nextQuestion () {
    ...
    currentAudio.value = await nextAudio.pipe(skip(store.exercise.currentQuestion - 1), take(1)).toPromise() as AudioResponse[]
  ...
  function isButtonDisabled (idx: number) {
    return inputDisabled.value || permutation.value.indexOf(idx) < currentIndex.value
  }
