<template>
  <div ref="buttons" class="row q-gutter-sm justify-center">
    <div v-for="(_, idx) in Array.from(Array(store.exercise.totalQuestions * 2).keys())" v-bind:key="idx" class="my-button-wrapper">
      <q-btn :color="isSelected(idx) ? 'accent' : 'primary'"
...
<script setup lang="ts">
  ...
  const { soundService, revealed, t, store, inputDisabled,route, difficulty } = createExerciseContext({
  ...
  onBeforeMount(() => {
    const numberOfQuestions = difficulty.value === 'easy' ? 6 : difficulty.value === 'normal' ? 12 : 20
  ...
  async function loadAudio (): Promise<void> {
    nextAudio.next(await new ExerciseService().fetchRandomWords({
      minLength: 3,
      maxLength: 14,
      lang: store.language,
  ...
  function isSelected (idx: number) {
    return idx === selectedButtonIdx.value || idx === secondSelectedButtonIdx.value
  }
  ...
  async function buttonClick (idx: number, $event: Event) {
    ...
    if (idx === selectedButtonIdx.value || idx === secondSelectedButtonIdx.value) {
    ...
    if (selectedButtonIdx.value > -1 && permutation[idx] === permutation[selectedButtonIdx.value]) {
      new SoundService().playSuccess()
      store.$patch(store => { store.exercise.correctAnswers++; store.exercise.currentQuestion++; })
    ...
    if (selectedButtonIdx.value === -1) {
      ...
      if (alreadyVisited.value.indexOf(idx) > -1 || alreadyVisited.value.indexOf(selectedButtonIdx.value) > -1) {
        useAppStore().strike()
