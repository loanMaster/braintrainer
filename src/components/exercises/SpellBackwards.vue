<template>
  <div ref="coreExercise" class="column">
    ...
    <div>
      <LetterButtons ref="letterButtons" @letter-selected="onLetterEntered" :numberOfButtons="6"/>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  import {ExerciseService, HomophoneAudioResponse} from "src/shared-services/exercise.service";
  ...
  async function nextQuestion () {
    ...
    homophones = (currentAudio.value as HomophoneAudioResponse).val.map(value => value.toUpperCase())
  ...
  function updateButtonLabels () {
    ...
    const previousLetter = currentIndex + 1 < homophones[0].length ? solution.value[currentIndex + 1] : ''
  ...
  async function loadNextAudio (): Promise<void> {
    nextAudio.next(await new ExerciseService().fetchHomophone({
      minLength: difficulty.value === 'easy' ? 3 : difficulty.value === 'normal' ? 5 : 7,
      maxLength: difficulty.value === 'easy' ? 5 : difficulty.value === 'normal' ? 9 : 16,
      lang: store.language,
      number: 1
    }))
  }
  ...
  function displayEmpty () {
    inputValue.value = '__________________________________'.substring(0, currentIndex + 1)
  ...
  function displayLetter (letter: string, hasError: boolean) {
    ...
    const spaces = '__________________________________'.substring(0, currentIndex)
  ...
  function matchesHomophone (letter: string): boolean {
    const text = letter + inputValue.value.replaceAll(' ', '').substring(currentIndex + 1)
    for (const homophone of homophones) {
