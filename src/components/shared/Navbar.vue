<template>
  <nav class="c-nav navbar navbar-expand-lg navbar-light bg-light c-shadow">
    <div class="container-fluid">
      <div
        class="g-desktop-hidden d-flex justify-content-start g-flex-1 user-select-none"
        @click="navigateTo({ name: '' })"
      >
        <img src="/favicon.ico" style="margin-right: 6px; height: 2rem" />
        <span class="d-flex align-items-center fw-bold g-mr-2 g-portrait-hidden"
          >BrainTrainer</span
        >
      </div>
      <span v-if="playerName" class="g-desktop-hidden g-mr-2">{{
        playerName
      }}</span>
      <FullScreenToggle class="g-mr-2 g-desktop-hidden" />
      <button
        class="navbar-toggler c-navbar-toggler"
        type="button"
        @click="collapsedMenu = !collapsedMenu"
      >
      ...
      >
        <img
          src="/favicon.ico"
          style="height: 2rem"
          class="g-mobile-hidden g-cursor-pointer user-select-none"
          @click="navigateTo({ name: '' })"
        />
        <span
          class="fw-bold g-mobile-hidden d-flex align-items-center g-cursor-pointer user-select-none"
          @click="navigateTo({ name: '' })"
          >BrainTrainer</span
        >
        <ul
        ...
        >
          <li class="nav-item">
            <AppRouterLink class="nav-link active" to="">{{
              $t('Play')
            }}</AppRouterLink>
          </li>
          <li class="nav-item">
            <AppRouterLink to="documentation">{{
              $t('Documentation')
            }}</AppRouterLink>
          </li>
          <li class="nav-item">
            <AppRouterLink class="nav-link active" to="highscores">{{
              $t('Highscores')
            }}</AppRouterLink>
          </li>
          <li class="nav-item">
            <AppRouterLink class="nav-link active" to="personalscores">{{
              $t('Your Scores')
            }}</AppRouterLink>
          </li>
          <li class="nav-item">
            <AppRouterLink class="nav-link active" to="users">{{
              $t('Users')
            }}</AppRouterLink>
          </li>
          <div
            class="g-desktop-hidden d-flex justify-content-center align-items-center mb-2 mt-2"
          >
            ...
            />
            <label
              class="btn btn-outline-dark g-mr-2 c-lang-select-button"
              for="lang-en"
              >en</label
            >
            <input
        ...
        <div class="g-mobile-hidden d-flex justify-content-end g-flex-1">
          <span v-if="playerName" class="d-flex align-items-center g-mr-2">{{
            playerName
          }}</span>
