<template>
  ...
  <div class="c-buttons mt-4 g-small-font">
    ...
    <div
      class="c-row mb-2 mt-2"
      v-for="player in getPlayers()"
      :key="player.id"
      :class="isCurrentUser(player.id) ? 'my-selected-player' : ''"
    >
      <button
        class="btn btn-outline-dark g-mr-2"
        @click="selectPlayer(player.id)"
        :disabled="isCurrentUser(player.id)"
      >
        {{ player.name }}
      </button>
      <button
        class="btn btn-outline-danger"
        @click="onDeleteUserClicked(player.id)"
        :disabled="getPlayerNumber() === 1"
      >
        🗑
      </button>
    </div>
    <div class="c-row mb-2 mt-2">
      <button
        class="btn btn-outline-dark g-mr-2"
        @click="onCreatePlayerClicked"
        :disabled="getPlayerNumber() >= 4"
      >
        {{ $t('Create user') }}
      </button>
      <span> </span>
