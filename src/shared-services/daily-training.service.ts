...
export class DailyTrainingService {
  getNextExercise (): { nameOfTheGame: string, difficulty: string } {
    ...
    const nextPossibleGames = GAMES.filter(g => lastPlayedGames.indexOf(g) === -1)

    const nextGame = nextPossibleGames[Math.floor(Math.random() * nextPossibleGames.length)]
    ...
    const nextDifficulty = averageHard > 1 || averageNormal > 2.5 ? 'hard' :
      averageNormal > 1 || averageEasy > 2.5 ? 'normal' : 'easy'
    return {  nameOfTheGame: nextGame, difficulty: nextDifficulty }
  }

  hasNext () {
    if (useAppStore().dailyTraining.results.find(r => r.nameOfTheGame.indexOf('MathMarathon') > -1)) {
