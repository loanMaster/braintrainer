...
export const calculateScore = (exercise: Exercise) => {
  ...
  if (percentageCorrect < 1.0 || numberOfErrors > 0) {
    return Math.max(
      Math.floor(
        percentageCorrect * baseScore +
          Math.min(8, numberOfErrors) * errorPenalty +
          percentageRemainingTime * 50
      ),
      100
    );
  } else {
    return Math.floor(
      baseScore +
        percentageRemainingTime * 275 +
        5 / Math.pow(1 - percentageRemainingTime, 2)
    );
  }
};
