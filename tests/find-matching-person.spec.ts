...
test('train find-matching-person', async ({ page }) => {
  ...
  for (let i = 0; i < 5; i++) {
    await expect(await page.locator('[data-testid="select-image-button"]:not([disabled])')).toHaveCount(1, { timeout: 30000 })
    const solution = await (await page.getByTestId('core-exercise').getAttribute('data-test'))
    await page.locator(`.q-carousel__thumbnail[src="${solution}"]`).click()
    await page.locator('[data-testid="select-image-button"]:not([disabled])').click()
  }
