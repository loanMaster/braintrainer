...
export const navigateToGame = async (page: Page, nameOfTheGame: string, difficulty: string) => {
  await page.goto('http://localhost:9000/en/train');
  ...
  await page.waitForURL('http://localhost:9000/en/train/select-difficulty/' + nameOfTheGame)
  await page.getByTestId('card-' + difficulty).click();
  await page.waitForURL(`http://localhost:9000/en/train/${difficulty}/${nameOfTheGame}`)
